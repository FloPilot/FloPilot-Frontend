...
export function PublicStorefrontView({ token }: { token: string }) {
  ...
  const isShowMode = isClientStoreShowMode(store);
  ...
  return (
    <div className="flex h-dvh max-h-dvh flex-col overflow-hidden bg-white">
      <StoreHeader
        ...
        accentHex={accentHex}
        cartCount={isShowMode ? undefined : cartCount}
        onCart={isShowMode ? undefined : openCart}
        ...
        actionSlot={
          isShowMode ? (
            <span className="inline-flex h-10 items-center gap-2 rounded-lg border border-[#e3e3e3] bg-white px-3 text-[13px] font-medium text-[#616161]">
              <Eye className="size-4" style={{ color: accentHex }} />
