...
import {
  ...
  INVOICE_HEADER_STYLE_DEFS,
  applyInvoiceDocumentTemplate,
  getInvoiceDocumentBlockDef,
  getInvoiceLineItemSectionDef,
  isBlockAbsorbedByHeader,
  missingInvoiceDocumentBlocks,
  missingInvoiceLineItemSections,
  normalizeInvoiceDocument,
  type InvoiceDocumentBlockId,
  type InvoiceDocumentSettings,
  type InvoiceHeaderStyle,
  type InvoiceLineItemSectionId,
...
function scrubAbsorbedBlocks(
  blocks: InvoiceDocumentBlockId[],
  headerStyle: InvoiceHeaderStyle
): InvoiceDocumentBlockId[] {
...
export function DocumentsInvoicesSection() {
  ...
  const { draft, setDraft, dirty, discard } = useSectionDraft<InvoiceDocumentSettings>(
    normalizeInvoiceDocument(settings.invoiceDocument)
  );
  ...
  const missingBlocks = missingInvoiceDocumentBlocks(
    draft.blocks,
    draft.headerStyle
  );
  const missingLineSections = missingInvoiceLineItemSections(
  ...
  const updateDraft = (patch: Partial<InvoiceDocumentSettings>) => {
    const next = normalizeInvoiceDocument({ ...draft, ...patch });
  ...
  const applyTemplate = (templateId: string) => {
    if (!isAdmin) return;
    setDraft(applyInvoiceDocumentTemplate(templateId));
  };

  const handleBlockDragEnd = (event: DragEndEvent) => {
    const { active, over } = event;
    if (!over || active.id === over.id) return;
    const oldIndex = draft.blocks.indexOf(active.id as InvoiceDocumentBlockId);
    const newIndex = draft.blocks.indexOf(over.id as InvoiceDocumentBlockId);
  ...
  const handleLineDragEnd = (event: DragEndEvent) => {
    ...
    const oldIndex = draft.lineItemSections.indexOf(
      active.id as InvoiceLineItemSectionId
    );
    const newIndex = draft.lineItemSections.indexOf(
      over.id as InvoiceLineItemSectionId
    );
    if (oldIndex < 0 || newIndex < 0) return;
    updateDraft({
  ...
  const handleSave = async () => {
    ...
    try {
      await updateSettings({
        invoiceDocument: normalizeInvoiceDocument(draft),
      });
      setSaved(true);
      window.setTimeout(() => setSaved(false), 2500);
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Could not save invoice layout"
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <main className="mx-auto w-full max-w-7xl space-y-8 p-4 sm:p-6 lg:p-8">
      <SettingsHeader
        ...
        description="Start from a proven layout, then fine-tune header composition, sections, and terms. Changes apply to every invoice PDF your shop sends."
      >
      ...
      <SettingsPanel
        ...
        description="Layouts inspired by common professional invoices — branded headers, from/to columns, and print-shop detail."
      >
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {INVOICE_DOCUMENT_TEMPLATES.map((template) => {
      ...
      <div className="grid items-start gap-8 xl:grid-cols-[minmax(0,1fr)_minmax(380px,0.95fr)]">
        <div className="space-y-6">
          <SettingsPanel
            ...
            description="Control where company information lives — under the logo, beside the invoice number, or in side-by-side From / To columns."
          >
            <div className="grid gap-2 sm:grid-cols-2">
              {INVOICE_HEADER_STYLE_DEFS.map((style) => {
              ...
              >
                <div className="space-y-2.5">
                  {visibleBlocks.map((blockId) => {
                    const def = getInvoiceDocumentBlockDef(blockId);
                    if (!def) return null;
                    return (
                      <SortableRow
                        key={blockId}
              ...
              >
                <div className="space-y-2.5">
                  {draft.lineItemSections.map((sectionId) => {
                    const def = getInvoiceLineItemSectionDef(sectionId);
                    if (!def) return null;
                    return (
                      <SortableRow
                        key={sectionId}
          ...
          <SettingsPanel
            title="Terms & payment"
            description="Footer copy on every invoice PDF. Logo and brand color still come from Appearance."
          >
            <div className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="invoice-terms" className="text-[#303030]">
                  Terms & notes
                </Label>
                <textarea
                  id="invoice-terms"
                ...
                <p className={dashboardTaskDetailClass}>
                  Payment and production notes for customers — keep these specific to invoices.
                </p>
              </div>
